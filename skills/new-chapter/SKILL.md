...
6. **Draft starter content** — before writing prose, read the voice reference for the target language (`voice-nl.md` for `nl`, `voice-en.md` for `en`, both in this skill's own folder) and match its traits. Then, after the header, write at least the first section with a short orientation paragraph and one concrete example (code block if appropriate), in each target language. Leave a `// TODO` comment where further sections are expected so the author knows where to continue. When scaffolding both languages, keep the section structure and examples identical between them — only the prose differs.
